import { _renderContext, ClippingNode, renderer } from '..'
...
export class ClippingNodeWebGLRenderCmd extends NodeWebGLRenderCmd {
  static _init_once = null
  static _visit_once = null
  static _layer = -1

  declare _node: ClippingNode
  declare _beforeVisitCmd: CustomRenderCmd
  declare _afterDrawStencilCmd: CustomRenderCmd
  declare _afterVisitCmd: CustomRenderCmd
  declare _currentStencilEnabled: boolean
  declare _mask_layer_le: number
  ...
  clippingVisit(parentCmd) {
    ...
    if (!node._stencil || !node._stencil.isVisible()) {
