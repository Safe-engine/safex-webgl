...
export const _LogInfos = {
  ...
  ActionManager_removeAction: 'safex: removeAction: Target not found',
  ...
  ActionManager_getActionByTag_2: 'safex: getActionByTag(tag = %s): Action not found',

  configuration_dumpInfo:
    'safex: **** WARNING **** CC_ENABLE_PROFILERS is defined. Disable it when you finish profiling (from ccConfig.js)',
  ...
  Director_resume: 'safex: Director: Error in gettimeofday',
  Director_setProjection: 'safex: Director: unrecognized projection',
  Director_popToSceneStackLevel: 'safex: Director: unrecognized projection',
  Director_popToSceneStackLevel_2: 'safex: Director: Error in gettimeofday',
  ...
  Node_getRotation: 'RotationX != RotationY. Dont know which one to return',
  Node_getScale: 'ScaleX != ScaleY. Dont know which one to return',
  Node_addChild: 'An Node cant be added as a child of itself.',
  Node_addChild_2: 'child already added. It cant be added again',
  ...
  Node_removeChildByTag_2: 'safex: removeChildByTag(tag = %s): child not found!',
  ...
  Node_initWithTexture: 'safex: Could not initialize AtlasNode. Invalid Texture.',

  AtlasNode_updateAtlasValues: 'AtlasNode.updateAtlasValues(): Shall be overridden in subclasses',
  AtlasNode_initWithTileFile: '',
  AtlasNode__initWithTexture: 'safex: Could not initialize AtlasNode. Invalid Texture.',
  ...
  __getListenerID: 'Dont call this method if the event is for touch.',

  eventManager__forceAddEventListener: 'Invalid scene graph priority!',
  eventManager_addListener: '0 priority is forbidden for fixed priority since its used for scene graph based priority.',
  eventManager_removeListeners: 'Invalid listener type!',
  eventManager_setPriority: 'cant set fixed priority with scene graph based listener.',
  ...
  eventManager_addListener_4: 'The listener has been registered, please Dont register it again.',
  ...
  animationCache__addAnimationsWithDictionary: 'safex: AnimationCache: No animations were found in provided dictionary.',
  ...
  animationCache__parseVersion1:
    'safex: AnimationCache: Animation -%s- found in dictionary without any frames - cannot add to animation cache.',
  animationCache__parseVersion1_2:
    'safex: AnimationCache: Animation -%s- refers to frame -%s- which is not currently in the SpriteFrameCache. This frame will not be added to the animation.',
  animationCache__parseVersion1_3:
    'safex: AnimationCache: None of the frames for animation -%s- were found in the SpriteFrameCache. Animation is not being added to the Animation Cache.',
  animationCache__parseVersion1_4:
    'safex: AnimationCache: An animation in your dictionary refers to a frame which is not in the SpriteFrameCache. Some or all of the frames for the animation -%s- may be missing.',
  animationCache__parseVersion2:
    'safex: CCAnimationCache: Animation -%s- found in dictionary without any frames - cannot add to animation cache.',
  ...
  Sprite__updateBlendFunc: 'Sprite._updateBlendFunc(): _updateBlendFunc doesnt work when the sprite is rendered using a CCSpriteBatchNode',
  ...
  SpriteBatchNode_increaseAtlasCapacity: 'safex: CCSpriteBatchNode: resizing TextureAtlas capacity from %s to %s.',
  SpriteBatchNode_increaseAtlasCapacity_2: 'safex: WARNING: Not enough memory to resize the atlas',
  SpriteBatchNode_reorderChild: 'SpriteBatchNode.addChild(): Child doesnt belong to Sprite',
  ...
  spriteFrameCache__getFrameConfig:
    'safex: WARNING: originalWidth/Height not found on the SpriteFrame. AnchorPoint wont work as expected. Regenrate the .plist',
  spriteFrameCache_addSpriteFrames: 'safex: WARNING: an alias with name %s already exists',
  spriteFrameCache__checkConflict: 'safex: WARNING: Sprite frame: %s has already been added by another source, please fix name conflit',
  spriteFrameCache_getSpriteFrame: 'safex: SpriteFrameCahce: Frame %s not found',
  ...
  TextureAtlas_initWithFile: 'safex: Could not open file: %s',
  ...
  textureCache_addUIImage: 'safex: Couldnt add UIImage in TextureCache',
  textureCache_dumpCachedTextureInfo: 'safex: -%s- id=%s %s x %s',
  textureCache_dumpCachedTextureInfo_2: 'safex: -%s- id= HTMLCanvasElement %s x %s',
  textureCache_dumpCachedTextureInfo_3: 'safex: TextureCache dumpDebugInfo: %s textures, HTMLCanvasElement for %s KB (%s MB)',
  ...
  Texture2D_initWithImage: 'safex: Texture2D. cant create Texture. UIImage is nil',
  Texture2D_initWithImage_2: 'safex: WARNING: Image (%s x %s) is bigger than the supported %s x %s',
  Texture2D_initWithString: 'initWithString isnt supported on safex-html5',
  ...
  Texture2D_bitsPerPixelForFormat: 'bitsPerPixelForFormat: %s, cannot give useful result, its a illegal pixel format',
  Texture2D__initPremultipliedATextureWithImage: 'safex: Texture2D: Using RGB565 texture since image has no alpha',
...
//   const doc = document
//   if (!logList) {
//     const logDiv = doc.createElement('Div')
...
//to make sure the log, warn, error and assert would not throw error before init by debugger mode.
export const _formatString = function (arg) {
  if (isObject(arg)) {
    try {
...
export const assert = function (cond, msg, url?, ...args) {
  if (!cond && msg) {
    for (const arg of args) {
      msg = msg.replace(/(%s)|(%d)/, _formatString(arg))
    }
    locLog(`Assert: ${msg}`)
  }
}
export const warn = function (...args: any[]) {
  locLog(`WARN :  ${formatStr(...args)}`)
}
export const log = function (...args: any[]) {
  locLog(formatStr(...args))
}
