 ...
 */
export const decode = function Jacob__Codec__Base64__decode(input) {
  const output = []
  let chr1,
    chr2,
    chr3,
 ...
 */
export const decodeAsArray = function Jacob__Codec__Base64___decodeAsArray(input, bytes) {
  const dec = this.decode(input)
  const ar = []
  let i, j, len
...
export const uint8ArrayToUint32Array = function (uint8Arr) {
