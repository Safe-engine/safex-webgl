...
export const Button = Widget.extend(/** @lends Button# */{
    ...
    _normalTexType: Widget.LOCAL_TEXTURE,
    _pressedTexType: Widget.LOCAL_TEXTURE,
    _disabledTexType: Widget.LOCAL_TEXTURE,
    ...
    _createSpriteFrameWithName: function (name) {
        ...
        if (frame == null) {
            cc.log("Scale9Sprite.initWithSpriteFrameName(): can't find the sprite frame by spriteFrameName");
            return null;
        }
        ...
        if (this._pressedTextureLoaded) {
            if (this.pressedActionEnabled){
                ...
                if(this._titleRenderer) {
                    ...
                    if (this._unifySize){
                        var zoomTitleAction = cc.scaleTo(Button.ZOOM_ACTION_TIME_STEP, 1, 1);
    ...
    _onPressStateChangedToPressed: function () {
        ...
        if (this._pressedTextureLoaded) {
            ...
            if (this.pressedActionEnabled) {
                ...
                var zoomAction = cc.scaleTo(Button.ZOOM_ACTION_TIME_STEP,
                                            1.0 + this._zoomScale,
                                            1.0 + this._zoomScale);
                ...
                if(this._titleRenderer) {
                    this._titleRenderer.stopAllActions();
                    this._titleRenderer.runAction(cc.scaleTo(Button.ZOOM_ACTION_TIME_STEP,
                                                             1 + this._zoomScale,
                                                             1 + this._zoomScale));
                }
            }
    ...
    _updateContentSize: function(){
        if (this._unifySize){
            if (this._scale9Enabled)
                ProtectedNode.setContentSize(this._customSize);
            else{
...
/**
 * allocates and initializes a UIButton.
 * @deprecated since v3.0, please use new Button() instead.
 ...
 * @return {Button}
 */
Button.create = function (normalImage, selectedImage, disableImage, texType) {
    return new Button(normalImage, selectedImage, disableImage, texType);
};
