  ...
  constructor() {
    ...
    const listener = (event: EventKeyboard) => {
      if ((event as any)._isPressed) {
        if ((this as any).onKeyPressed) (this as any).onKeyPressed((event as any)._keyCode, event)
      } else {
        if ((this as any).onKeyReleased) (this as any).onKeyReleased((event as any)._keyCode, event)
      }
    }
