...
export class DrawingPrimitiveWebGL {
  ...
  constructor(ctx?: WebGLRenderingContext) {
    ...
    if (!(ctx instanceof WebGLRenderingContext)) {
      throw new Error('Cant initialize DrawingPrimitiveWebGL. context need is WebGLRenderingContext')
    }
  ...
  drawSolidRect(origin: Point, destination: Point, color: Color) {
    const vertices = [origin, { x: destination.x, y: origin.y }, destination, { x: origin.x, y: destination.y }]
  ...
  drawPoly(vertices: Point[], numOfVertices: number, closePolygon: boolean) {
    ...
    if (closePolygon) glContext.drawArrays(glContext.LINE_LOOP, 0, vertices.length)
