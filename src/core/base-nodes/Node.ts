...
export class Node extends EventHelper {
  ...
  getComponent(name: any) {
    if (this._componentContainer) return this._componentContainer.getComponent(name)
    return null
  }
  ...
  removeComponent(component: any) {
    if (this._componentContainer) return this._componentContainer.remove(component)
    return false
  }
  removeAllComponents() {
  ...
  transform(parentCmd?: any, recursive?: any) {
  ...
  getBoundingBoxToWorld() {
    ...
    for (let i = 0; i < locChildren.length; i++) {
      ...
      if (child && child._visible) {
        ...
        if (childRect) r = rectUnion(r, childRect)
      }
    }
    return r
  }
