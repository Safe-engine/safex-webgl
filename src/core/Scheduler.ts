...
class ListEntry {
  ...
  static put(entry: ListEntry) {
    ...
    if (ListEntry._pool.length < MAX_POOL_SIZE) ListEntry._pool.push(entry)
  }
}
...
class HashUpdateEntry {
  ...
  static put(entry: HashUpdateEntry) {
    ...
    entry.target = null
    entry.callback = null
    if (HashUpdateEntry._pool.length < MAX_POOL_SIZE) HashUpdateEntry._pool.push(entry)
  }
}
...
class HashTimerEntry {
  ...
  static put(entry: HashTimerEntry) {
    ...
    entry.paused = false
    if (HashTimerEntry._pool.length < MAX_POOL_SIZE) HashTimerEntry._pool.push(entry)
  }
}
...
class CallbackTimer {
  ...
  static put(timer: CallbackTimer) {
    ...
    timer._key = null
    if (CallbackTimer._pool.length < MAX_POOL_SIZE) CallbackTimer._pool.push(timer)
  }
}
...
export class Scheduler {
  ...
  update(dt: number) {
    ...
    for (i = 0, list = this._updatesNegList; i < list.length; ) {
      entry = list[i]
      if (entry.markedForDeletion) this._removeUpdateFromHash(entry)
      else i++
    }
