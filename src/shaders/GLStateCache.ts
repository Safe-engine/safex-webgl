...
import { BLEND_DST, BLEND_SRC } from '../core/platform/Macro'
...
if (ENABLE_GL_STATE_CACHE) {
  ...
  _blendingDest = -1
  // _GLServerState = 0
  if (TEXTURE_ATLAS_USE_VAO) _uVAO = 0
}
...
export const glInvalidateStateCache = function () {
  ...
  if (ENABLE_GL_STATE_CACHE) {
    _currentShaderProgram = -1
    for (let i = 0; i < MAX_ACTIVETEXTURE; i++) {
...
export const glDeleteProgram = function (program) {
  if (ENABLE_GL_STATE_CACHE) {
    if (program === _currentShaderProgram) _currentShaderProgram = -1
  }
 ...
 * @param {Number} dfactor
 */
export const glBlendFunc = ENABLE_GL_STATE_CACHE
...
export const glBlendResetToCache = function () {
  ...
  if (ENABLE_GL_STATE_CACHE) setBlending(_blendingSource, _blendingDest)
  else setBlending(BLEND_SRC, BLEND_DST)
}
 ...
 * @param {Texture2D} textureId
 */
export const glBindTexture2DN = ENABLE_GL_STATE_CACHE
  ? function (textureUnit, textureId) {
      if (_currentBoundTexture[textureUnit] === textureId) return
...
export const glDeleteTextureN = function (textureUnit, textureId) {
  if (ENABLE_GL_STATE_CACHE) {
    if (textureId === _currentBoundTexture[textureUnit]) _currentBoundTexture[textureUnit] = -1
  }
