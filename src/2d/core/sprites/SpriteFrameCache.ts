...
export const spriteFrameCache = {
  ...
  _addSpriteFramesByObject: function (url, jsonObject, texture) {
    ...
    const frameConfig = this._frameConfigCache[url] || this._getFrameConfigByJsonObject(url, jsonObject)
      ...
      if (!spriteFrame) {
        ...
        if (aliases) {
          ...
          for (let i = 0, li = aliases.length; i < li; i++) {
            ...
            if (spAliases[alias]) log(_LogInfos.spriteFrameCache_addSpriteFrames, alias)
            spAliases[alias] = key
          }
        }
  ...
  addSpriteFrames: function (url, texture?) {
    ...
    const dict = this._frameConfigCache[url] || loader.getRes(url)
    if (!dict || !dict['frames']) return

    const frameConfig = this._frameConfigCache[url] || this._getFrameConfig(url)
    //this._checkConflict(frameConfig);                             //TODO
    this._createSpriteFrames(url, frameConfig, texture)
  },
  ...
  _checkConflict: function (dictionary) {
    const framesDict = dictionary['frames']

    for (const key in framesDict) {
