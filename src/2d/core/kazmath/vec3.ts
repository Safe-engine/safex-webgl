...
var _p = Vec3.prototype;

_p.fill = function (x, y, z) {    // =kmVec3Fill
  if (x && y === undefined) {
    this.x = x.x;
    this.y = x.y;
    this.z = x.z;
...
_p.cross = function (vec3) {   //= kmVec3Cross
...
_p.dot = function (vec) {     //= kmVec3Dot
  return (this.x * vec.x + this.y * vec.y + this.z * vec.z);
};

_p.add = function (vec) {      //= kmVec3Add
  this.x += vec.x;
  this.y += vec.y;
  this.z += vec.z;
  return this;
...
_p.subtract = function (vec) {  // = kmVec3Subtract
  this.x -= vec.x;
  this.y -= vec.y;
  this.z -= vec.z;
  return this;
...
_p.transform = function (mat4) {             // = kmVec3Transform
...
_p.transformCoord = function (mat4) {        // = kmVec3TransformCoord
  /*
   a = (Vx, Vy, Vz, 1)
   b = (a×M)T
   Out = 1⁄bw(bx, by, bz)
...
_p.scale = function (scale) {             // = kmVec3Scale
  this.x *= scale;
  this.y *= scale;
  this.z *= scale;
  return this;
...
_p.equals = function (vec) {    // = kmVec3AreEqual
  var EPSILON = EPSILON;
...
_p.inverseTransform = function (mat4) {   //= kmVec3InverseTransform
  var mat = mat4.mat;
...
_p.inverseTransformNormal = function (mat4) {   // = kmVec3InverseTransformNormal
  var x = this.x, y = this.y, z = this.z, mat = mat4.mat;
...
Vec3.zero = function (vec) {   // = kmVec3Zero
  vec.x = vec.y = vec.z = 0.0;
  return vec;
};

_p.toTypeArray = function () {           //kmVec3ToTypeArray
