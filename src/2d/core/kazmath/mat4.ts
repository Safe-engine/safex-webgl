...
export namespace math {
  ...
  export let kmMat4 = Matrix4;
  let proto = Matrix4.prototype;

  /**
   * Fills a Matrix4 structure with the values from a 16 element array of floats
   * @param {Array} scalarArr
   */
  proto.fill = function (scalarArr) {        //kmMat4Fill
    var mat = this.mat;
    for (var i = 0; i < 16; i++) {
      mat[i] = scalarArr[i];
    }
  ...
  var identityMatrix = new Matrix4().identity();
  /**
  ...
  export const kmMat4Inverse = function (pOut, pM) {
    var inv = new Matrix4(pM);
    var tmp = new Matrix4(identityMatrix);
    if (Matrix4._gaussj(inv, tmp) === false)
      return null;
    pOut.assignFrom(inv);
    return pOut;
  };
   ...
   * @returns {Matrix4} Returns null if there is no inverse, else returns a new inverse matrix object
   */
  proto.inverse = function () {    //kmMat4Inverse
    var inv = new Matrix4(this);
    var tmp = new Matrix4(identityMatrix);
    if (Matrix4._gaussj(inv, tmp) === false)
      return null;
    return inv;
   ...
   */
  proto.isIdentity = function () {       // kmMat4IsIdentity
    var mat = this.mat;
   ...
   */
  proto.transpose = function () {        // kmMat4Transpose
    var mat = this.mat;
  ...
  proto.equals = function (mat4) {
    ...
    var matA = this.mat, matB = mat4.mat, EPSILON = EPSILON;
    for (var i = 0; i < 16; i++) {
   ...
   * @returns {Matrix4}
   */
  Matrix4.createByRotationX = function (radians, matrix) {    //kmMat4RotationX
    /*
     |  1  0       0       0 |
     M = |  0  cos(A) -sin(A)  0 |
     |  0  sin(A)  cos(A)  0 |
   ...
   * @returns {*}
   */
  Matrix4.createByRotationY = function (radians, matrix) {      // kmMat4RotationY
    /*
     |  cos(A)  0   sin(A)  0 |
     M = |  0       1   0       0 |
     | -sin(A)  0   cos(A)  0 |
   ...
   * @return {Matrix4}
   */
  Matrix4.createByRotationZ = function (radians, matrix) {    // kmMat4RotationZ
    /*
     |  cos(A)  -sin(A)   0   0 |
     M = |  sin(A)   cos(A)   0   0 |
     |  0        0        1   0 |
   ...
   * @returns {Matrix4}
   */
  Matrix4.createByPitchYawRoll = function (pitch, yaw, roll, matrix) {
    matrix = matrix || new Matrix4();
   ...
   * @returns {Matrix4}
   */
  Matrix4.createByQuaternion = function (quaternion, matrix) {
    matrix = matrix || new Matrix4();
    var mat = matrix.mat;
   ...
   * @param {math.Vec3} translation
   * @param {Matrix4} [matrix] if matrix is undefined, creates a new matrix.
   * @returns {Matrix4}
   */
  Matrix4.createByRotationTranslation = function (rotation, translation, matrix) {
    matrix = matrix || new Matrix4();
   ...
   * @returns {Matrix4}
   */
  Matrix4.createByScale = function (x, y, z, matrix) {  //kmMat4Scaling
    matrix = matrix || new Matrix4();
    var mat = matrix.mat;
    mat[0] = x;
    mat[5] = y;
   ...
   * @returns {Matrix4}
   */
  Matrix4.createByTranslation = function (x, y, z, matrix) {       //kmMat4Translation
    matrix = matrix || new Matrix4();
    matrix.identity();
    matrix.mat[12] = x;
    matrix.mat[13] = y;
   ...
   *  @returns {math.Vec3}
   */
  proto.getUpmath.Vec3 = function () {
    var mat = this.mat;
    var ret = new math.Vec3(mat[4], mat[5], mat[6]);
   ...
   * @returns {math.Vec3}
   */
  proto.getRightmath.Vec3 = function () {
    var mat = this.mat;
    var ret = new math.Vec3(mat[0], mat[1], mat[2]);
    return ret.normalize();
  };
   ...
   * @returns {math.Vec3}
   */
  proto.getForwardmath.Vec3 = function () {
    var mat = this.mat;
    var ret = new math.Vec3(mat[8], mat[9], mat[10]);
    return ret.normalize();
  };
   ...
   */
  Matrix4.createPerspectiveProjection = function (fovY, aspect, zNear, zFar) {
    var r = degreesToRadians(fovY / 2), deltaZ = zFar - zNear;
    var s = Math.sin(r);
    ...
    //cos(r) / sin(r) = cot(r)
    var cotangent = Math.cos(r) / s;
    var matrix = new Matrix4(), mat = matrix.mat;
    matrix.identity();
   ...
   * @returns {Matrix4}
   */
  Matrix4.createOrthographicProjection = function (left, right, bottom, top, nearVal, farVal) {
    var matrix = new Matrix4(), mat = matrix.mat;
    matrix.identity();
    mat[0] = 2 / (right - left);
    mat[5] = 2 / (top - bottom);
  ...
  export const kmMat4LookAt = function (pOut, pEye, pCenter, pUp) {
    var f = new math.Vec3(pCenter), up = new math.Vec3(pUp);
    f.subtract(pEye);
    f.normalize();
    up.normalize();

    var s = new math.Vec3(f);
    s.cross(up);
    s.normalize();

    var u = new math.Vec3(s);
    u.cross(f);
    s.normalize();
    ...
    var translate = Matrix4.createByTranslation(-pEye.x, -pEye.y, -pEye.z);
  ...
  proto.lookAt = function (eyeVec, centerVec, upVec) {
    var f = new math.Vec3(centerVec), up = new math.Vec3(upVec), mat = this.mat;
    f.subtract(eyeVec);
    f.normalize();
    up.normalize();
    ...
    tempMatrix = Matrix4.createByTranslation(-eyeVec.x, -eyeVec.y, -eyeVec.z, tempMatrix);
  ...
  export const kmMat4RotationAxisAngle = function (pOut, axis, radians) {
    ...
    var normalizedAxis = new math.Vec3(axis);
   ...
   * @param {math.Vec3} axis
   * @param {Number} radians
   * @param {Matrix4} [matrix]
   * @returns {Matrix4}
   */
  Matrix4.createByAxisAndAngle = function (axis, radians, matrix) {
    matrix = matrix || new Matrix4();
  ...
  proto.extractRotation = function () {
    var matrix = new math.Matrix3(), mat4 = this.mat, mat3 = matrix.mat;
    mat3[0] = mat4[0];
    mat3[1] = mat4[1];
    mat3[2] = mat4[2];
  ...
  proto.extractPlane = function (planeType) {
    var plane = new math.Plane(), mat = this.mat;
    switch (planeType) {
      case math.Plane.RIGHT:
        plane.a = mat[3] - mat[0];
        plane.b = mat[7] - mat[4];
        plane.c = mat[11] - mat[8];
        plane.d = mat[15] - mat[12];
        break;
      case math.Plane.LEFT:
        plane.a = mat[3] + mat[0];
        plane.b = mat[7] + mat[4];
        plane.c = mat[11] + mat[8];
        plane.d = mat[15] + mat[12];
        break;
      case math.Plane.BOTTOM:
        plane.a = mat[3] + mat[1];
        plane.b = mat[7] + mat[5];
      ...
      case math.Plane.NEAR:
        ...
        break;
      default:
        log("Matrix4.extractPlane: Invalid plane index");
        break;
    }
